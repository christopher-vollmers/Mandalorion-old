...
os.system('python3 %s/Mandalorion_5_TESS.py %s %s %s %s %s' %(scripts,content_file, path,'0.02', genome_annotation, refine)) # This script identifies TSS and TES. Output file 'TESS.bed' is written to 'path'. The last argument determines how stringent the script filters potential TSS/TES. The lower the more stringent. 0.03 is our default
os.system('python3 %s/Mandalorion_6_SS.py %s %s %s %s %s' %(scripts,content_file, path,'0.05', genome_annotation, refine)) # This script identifier splice sites. Output file 'SS.bed' is written to 'path'.
if 'i' in refine:
    os.system('python3 %s/Mandalorion_6.5_Refine_SS.py %s %s %s %s' %(scripts,illumina_content_file,genome_annotation, path,refine)) # This script refines splice sites using genome annotation and/or Illumina data. Output file 'SS_refined.bed' is written to 'path'.
else:
...
os.system('python3 %s/Mandalorion_7_Gene_Expression.py %s %s %s' %(scripts,content_file,path,genome_annotation)) # This script determine gene-level expression. Requires >8G of RAM for mouse/human genome. All other outputs are independent of this script so you can just comment it out if you are not interested in gene level data.
os.system('python3 %s/Mandalorion_8_Combined_TESS_SS.py %s %s ' %(scripts,content_file,path)) # This script groups TSS/TES and SS if they are linked by raw reads
os.system('python3 %s/Mandalorion_9_Match_TESS_SS_Combinations_to_genes.py %s %s ' %(scripts,genome_annotation,path)) # This script matches the groups wit annotated genes
os.system('python3 %s/Mandalorion_10_Determine_Alternative_Splicing.py %s %s' %(scripts,content_file,path)) # This script looks at raw read alignments to determine alt. splicing and intron retentions
os.system('python3 %s/Mandalorion_11_Define_and_Quantify_Isoforms.py %s %s %s %s' %(scripts,content_file, path,40,0)) # This script sort raw reads into isoform bins. The two number variables determine the window around TSS and TES in which read ends can fall and still be matched to the site.
